...
- 건축물 용도 검증: 건축HUB 표제부에 도시형생활주택·소형주택이 명시되면 제외
...
이때 기존 카탈로그가 새 조건의 상위집합이면 기존 후보만 재검증해 서울 전체 목록을
다시 수집하지 않습니다. `refresh_catalog`를 직접 켠 경우에만 법정동부터 전부 갱신합니다.
K-apt가 단지분류를 단순히 아파트로 제공하는 경우가 있어, 실거래 연결 후보는 건축HUB
표제부의 주용도·기타용도까지 확인합니다. `도시형생활주택` 또는 `소형주택`이 확인되면
최종 카탈로그에서 제외합니다.

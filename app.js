const APP_VERSION = "v2026.08.06-01-rc.7";
...
const SUPPLY_CALCULATION_VERSION = "supply-model-v11-permit-discovery-first";
...
function renderApiKeyStatus() {
  const showSettingsButton = shouldShowApiSettings();
  el.openSettingsButton.hidden = !showSettingsButton;
  el.openSettingsButton.classList.toggle("is-hidden", !showSettingsButton);
...
function shouldShowApiSettings() {
  const adminMode = new URLSearchParams(window.location.search).get(ADMIN_QUERY_PARAM);
  if (adminMode === "1") return true;
  if (adminMode === "0") return false;
  return isLocalExecution();
}

function isLocalExecution() {
  const host = window.location.hostname;
  return window.location.protocol === "file:" || !host || ["localhost", "127.0.0.1", "::1"].includes(host);
}
